...
export default async function PublicMenuLayout({
  ...
  return (
    <div className="min-h-screen bg-neutral-50 flex flex-col font-sans text-neutral-800 antialiased selection:bg-[#FD7E14]/10">
      <div className="relative h-40 md:h-56 w-full overflow-hidden bg-neutral-900">
        <Image
          src={coverUrl}
          alt={`Capa de ${establishment.name}`}
          fill
          priority
          className="object-cover opacity-60 scale-105 blur-[2px] transition-all duration-700 hover:scale-100 hover:blur-0"
        />
        <div className="absolute inset-0 bg-gradient-to-t from-neutral-900 via-neutral-950/20 to-transparent" />
      </div>

      <header className="relative z-10 -mt-12 px-4 max-w-4xl mx-auto w-full">
